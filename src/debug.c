...
noreturn void fatal_error(const char *file, int line, const char *msg, int err)
{
	log_msg(LOG_CLASS_ERROR, "debug", file, line, "%s: %s", msg,
		strerror(err));
	print_stack_trace();
	fflush(stderr);
	abort();
...
noreturn void uv_fatal_error(const char *file, int line, char *msg, int err)
{
	...
	log_msg(LOG_CLASS_ERROR, "debug", file, line, "%s: libuv error(%s): %s",
		msg, err_name, strerr);
	print_stack_trace();
	fflush(stderr);
	abort();
...
bug(const char *file, int line, const char *func, const char *fmt, ...)
{
	...
	log_msg(LOG_CLASS_ERROR, "debug", file, line, "BUG in %s(): %s", func,
		msg);
	print_stack_trace();
	fflush(stderr);
	abort();
